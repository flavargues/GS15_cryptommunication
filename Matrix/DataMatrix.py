...
class DataMatrix :
    def __init__(self, rows_number, columns_number, data):
        ...
        for b in range(len(data)):
            ...
            if colIndex==0: self.rows.append([])
    ...
    def shiftRows(self) :
        result = DataMatrix(self.rows_number,self.columns_number, b"                ")
        for i in range(self.rows_number):
            for j in range(self.columns_number):
    ...
    def mixSingleColumn(self, colId, constantMatrix):
        for constRow in range(constantMatrix.rows_number):
            totalValue = applyIrrPolTheo(constantMatrix.rows[constRow][0] * self.rows[0][colId])
            for constCol in range(1,self.rows_number):
                totalValue ^= applyIrrPolTheo(constantMatrix.rows[constRow][constCol] * self.rows[constCol][colId])
            print(totalValue)
...
def main():
    dataMatrix=DataMatrix(4,4,b"abcdefghijklmnop")
    keyMatrix=DataMatrix(4,4,b"hdsjkdhhdsmqppws")
    constantMatrix=DataMatrix(4,4,[
        0x02,0x03,0x01,0x01,
        0x01,0x02,0x03,0x01,
        0x01,0x01,0x02,0x03,
        0x03,0x01,0x01,0x02
